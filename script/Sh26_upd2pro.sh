...
if [ "$upd2pro_enable" != "0" ] || [ "$upd2pro2_enable" != "0" ] ; then
...
B_restart="$upd2pro_enable$upd2pro2_enable$upd2pro_path$upd2pro2_path$(cat /etc/storage/app_3.sh /etc/storage/app_4.sh | grep -v '^#' | grep -v "^$")"
...
if  [ "$needed_restart" = "1" ] ; then
	...
	[ "$upd2pro_enable" != "1" ] && [ "$upd2pro2_enable" != "1" ] && { eval $(ps -w | grep "$scriptname" | grep -v grep | awk '{print "kill "$1";";}'); exit 0; }
fi
if [ "$upd2pro_enable" = "1" ] || [ "$upd2pro2_enable" = "1" ] ; then
	if [ "$needed_restart" = "1" ] ; then
		upd2pro_close
		upd2pro_start
	else
...
while [ "$upd2pro_enable" = "1" ] || [ "$upd2pro2_enable" = "1" ]; do
...
sed -Ei '/【upd2pro】|^$/d' /tmp/script/_opt_script_check
# restart_dhcpd
killall app_3.sh app_4.sh udp2raw speeder
killall -9 app_3.sh app_4.sh udp2raw speeder
...
eval $upd2pro2_path &
sleep 2
[ ! -z "$(ps -w | grep "/opt/bin/speeder" | grep -v grep )" ] && logger -t "【upd2pro】" "启动成功 speeder $upd2pro_v " && upd2pro_restart o
...
[ "$1" = "del1" ] || [ "$1" = "del2" ] && /etc/storage/www_sh/upd2pro del &
}
