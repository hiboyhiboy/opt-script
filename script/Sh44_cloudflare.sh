...
arDdnsCheck() {
	...
	if [ "$hostIP"x = "x"  ] ; then
		curltest=`which curl`
		if [ -z "$curltest" ] || [ ! -s "`which curl`" ] ; then
			hostIP=`wget --no-check-certificate --quiet --output-document=- "http://www.ipip.net" | grep "IP地址" | grep -E -o '([0-9]+\.){3}[0-9]+'`
		else
			hostIP=`curl -L -k -s "http://www.ipip.net" | grep "IP地址" | grep -E -o '([0-9]+\.){3}[0-9]+'`
		fi
		if [ "$hostIP"x = "x"  ] ; then
