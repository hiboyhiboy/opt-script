...
B_restart="$cloudflare_enable$cloudflare_Email$cloudflare_Key$cloudflare_domian$cloudflare_host$cloudflare_domian2$cloudflare_host2$cloudflare_interval$(cat /etc/storage/ddns_script.sh | grep -v '^#' | grep -v "^$")"
...
if [ "$cloudflare_domian2"x != "x" ] && [ "$cloudflare_domian2" != "baidu.com" ] ; then
	sleep 1
...
recordIP=$(curl -k -s -X GET "https://api.cloudflare.com/client/v4/zones/$Zone_ID/dns_records" \
     ...
     -H "X-Auth-Key: $cloudflare_Key" \
     -H "Content-Type: application/json")
recordIP=$(echo $recordIP|grep -o "name\":\"$domian_tmp.$host_tmp\",\"content\":\"[0-9\.]*\""| awk -F content '{print $2}'|grep -o "[0-9\.]*")
	case "$recordIP" in 
	[1-9][0-9]*)
		echo $recordIP
		return 0
...
if [ "$RECORD_ID" = "" ] ; then
	...
	RESULT==$(curl -k -s -X POST "https://api.cloudflare.com/client/v4/zones/$Zone_ID/dns_records" \
     ...
     -H "X-Auth-Key: $cloudflare_Key" \
     -H "Content-Type: application/json" \
     --data '{"type":"A","name":"'$domian_tmp'","content":"'$hostIP'","ttl":120,"proxied":false}')
