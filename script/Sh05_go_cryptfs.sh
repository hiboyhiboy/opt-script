...
if [ ! -z "$(echo $scriptfilepath | grep -v "/tmp/script/" | grep cry_fs)" ]  && [ ! -s /tmp/script/_app23 ]; then
	mkdir -p /tmp/script
...
# 启动 gocryptfs 后可以使用 /tmp/AiDisk_00/lockdir 路径防止硬盘盗窃导致的数据泄露
# 注意事项: 启动前需加密使用的文件夹要为空；启动前需手动在ssh终端运行命令初始化配置。
if [ ! -f "/tmp/AiDisk_00/lockdir_gocryptfsdata/gocryptfs.conf" ] ; then
    logger -t "【gocryptfs】" "启动前需手动在ssh终端运行命令初始化配置 gocryptfs  -init /tmp/AiDisk_00/lockdir_gocryptfsdata" ; sleep 120 ;
fi
killall gocryptfs
modprobe fuse
gocryptfs_pass=$1
fusermount -u /tmp/AiDisk_00/lockdir
echo $gocryptfs_pass | gocryptfs /tmp/AiDisk_00/lockdir_gocryptfsdata /tmp/AiDisk_00/lockdir_gocryptfs
...
if [ "$gocryptfs_key_enable" = "2" ] && [ -z "$gocryptfs_pass" ] ; then
